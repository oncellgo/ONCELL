...
### 2.4 일정 구분(카테고리) 색상 · 순서 (필수)

일정 달력·주간 뱃지·기타설정의 구분 관리 모두 **단일 출처**(`lib/categoryColors.ts`)의 팔레트·순서를 사용한다.

**색상 (연한 계열 5종 + 기본)**

| 그룹 | 포함 구분 | bg | fg | border |
|---|---|---|---|---|
| 예배 (연빨강) | 일반예배 · 특별예배 | `#FEE2E2` | `#991B1B` | `#FCA5A5` |
| 기도회 (연오렌지) | 기도회 · 특별기도회 | `#FFEDD5` | `#9A3412` | `#FDBA74` |
| 양육 (연초록) | 양육 · 수련회 · 교육 | `#DCFCE7` | `#166534` | `#86EFAC` |
| 기념일 (회색) | 기념일 | `#F3F4F6` | `#4B5563` | `#D1D5DB` |
| 행사 (연노랑) | 행사 | `#FEF9C3` | `#854D0E` | `#FDE047` |
| 기타 (기본 라임) | 사용자 추가 구분 | `#ECFCCB` | `#3F6212` | `#D9F09E` |

**표시 순서 (고정)**: `예배 → 기도회 → 양육 → 기념일 → 행사 → (기타)`.
같은 그룹 내에서는 `일반예배 → 특별예배`, `기도회 → 특별기도회` 처럼 **일반 → 특별** 순. 달력 범례·구분 pill 목록·이벤트 모달의 select 옵션은 모두 이 순서를 따른다.

**적용 포인트**
- 월달력 셀 배경·다중 카테고리 분할 (`pages/management.tsx`의 `colorFor`)
- 주간 카드 상단 예배 뱃지(`badgePalette`) · 주간 카드 불릿 점
- 기타설정의 "일정 구분 관리" pill + 달력 색상 안내 범례 (`components/EtcSettings.tsx`)
- 새 일정 등록 모달의 구분 select

**금지 사항**
- 카테고리별로 `#DBEAFE`, `#FEF3C7`, `#FCE7F3` 등 다른 hex 조합을 개별 화면에서 재정의 금지
- 임의 순서(알파벳·삽입순·DB 반환순)로 카테고리를 나열 금지 — 반드시 `sortCategories()` 경유
- 새 구분 그룹을 추가하려면 `lib/categoryColors.ts` + 본 규칙 표를 **함께 업데이트**
